            ...
            if _main_loop and not _main_loop.is_closed():
                try:
                    asyncio.run_coroutine_threadsafe(
                        broadcast_update(event.src_path),
                        _main_loop
                    )
                except Exception as e:
    ...
    try:
        ...
        for pattern in SECRET_PATTERNS:
            ...
            for match in matches:
                ...
                false_positives = ['false', 'null', 'none', '""', "''", '', 'true', '0', '1', 
                                 'redacted', '[redacted]', 'n/a', 'na', 'none', 'null']
                if (value.lower() not in false_positives and 
                    len(value) >= 8 and 
                    not value.startswith('[') and  # Skip [REDACTED] patterns
...
async def get_chain(underlying: str):
    ...
    try:
        ...
        if not chain_file.exists():
            return {"underlying": underlying, "contracts": [], "message": "Chain data not found", "spot": 0, "pcr": 1.0, "total_contracts": 0, "data_source": "real"}
        ...
        if df is None:
            ...
            if not rows:
                return {"underlying": underlying, "contracts": [], "message": "Chain file is empty", "spot": 0, "pcr": 1.0, "total_contracts": 0}
            ...
            if 'status' in df_dict:
                status_rows = [i for i, s in enumerate(df_dict['status']) if s and str(s).strip() not in ['', 'nan', 'None']]
            ...
            if 'underlying' in df_dict:
                ...
                return {"underlying": underlying, "contracts": [], "message": "No underlying column in chain data", "spot": 0, "pcr": 1.0, "total_contracts": 0}
            
            if not filtered_rows:
                return {"underlying": underlying, "contracts": [], "message": "No data for this underlying", "spot": 0, "pcr": 1.0, "total_contracts": 0}
        ...
        if 'volume' in df.columns and 'oi' in df.columns:
            df['liquidity_score'] = pd.to_numeric(df['volume'], errors='coerce').fillna(0) * 0.4 + pd.to_numeric(df['oi'], errors='coerce').fillna(0) * 0.6
        else:
...
async def get_performance():
    ...
    try:
        ...
        if not perf_file.exists():
            return {"status": "NO_DATA", "reason": "perf_metrics.json not found", "current": {}, "history": [], "data_source": "real"}
        ...
        try:
            ...
            history = [
                {
                    "timestamp": row[0],
                    "cycle_duration": row[1],
                    "fetch_duration": row[2],
                    "strategy_duration": row[3]
                }
                for row in rows
            ]
        except:
    ...
    try:
        ...
        try:
            ...
            return {
                "signals": [],
                "count": 0,
                "message": f"Failed to read signal CSV: {str(e)}",
                "data_source": "real"
            }
...
async def validate_data():
    ...
    try:
        ...
        if validator_path.exists():
            ...
            return {
                "status": "error",
                "message": "Validator script not found"
            }
    except Exception as e:
...
async def comprehensive_audit():
    ...
    try:
        ...
        # Save audit result
        audit_file = AUDIT_DIR / f"comprehensive_audit_{datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y%m%d_%H%M%S')}.json"
        ...
        if not historical_data:
            ...
            for i in range(100):
                historical_data.append({
                    "timestamp": (datetime.now(pytz.timezone('Asia/Kolkata')) - timedelta(days=100-i)).isoformat(),
    ...
    try:
        if not ADVANCED_FEATURES_AVAILABLE:
            return {
                ...
                "performance": {
                    "models": {},
                    "message": "ML tracking not available"
                }
            }
        ...
        try:
            ...
            return {
                "status": "ok",
                "performance": performance if performance else {"models": {}}
            }
            ...
            return {
                ...
                "performance": {
                    "models": {},
                    "message": f"ML tracker error: {str(tracker_error)[:200]}"
                }
            }
    except Exception as e:
        return {
...
async def compare_ml_models():
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "ok",
                "comparison": comparison if comparison else {"models": {}}
            }
        except Exception as tracker_error:
            # Fallback if ML tracker fails
            return {
    ...
    try:
        if not UPGRADE_AGENT_AVAILABLE:
            return {
                "status": "ok",
                "available": False,
                "message": "Upgrade agent not available",
                "paused": False
            }
...
async def runner_stop():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, str(runner_script), "stop"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=str(ROOT_DIR)
