...
class AdvancedFiltering:
    ...
    def filter_option_chain(
        ...
        if 'strike_range' in filters:
            ...
            filtered = [
                c for c in filtered
                if min_strike <= c.get('strike', 0) <= max_strike
            ]
        ...
        if 'expiries' in filters and filters['expiries']:
            filtered = [
                c for c in filtered
                if c.get('expiry') in filters['expiries']
            ]
        ...
        if filters.get('near_atm', False) and contracts:
            ...
            if spot > 0:
                ...
                filtered = [
                    c for c in filtered
                    if abs(c.get('strike', 0) - spot) <= atm_range
                ]
        ...
        if 'volume_min' in filters:
            filtered = [
                c for c in filtered
                if c.get('volume', 0) >= filters['volume_min']
            ]
        ...
        if 'delta_range' in filters:
            ...
            filtered = [
                c for c in filtered
                if min_delta <= c.get('delta', 0) <= max_delta
            ]
        ...
        if 'gamma_range' in filters:
            ...
            filtered = [
                c for c in filtered
                if min_gamma <= c.get('gamma', 0) <= max_gamma
            ]
        ...
        if 'option_types' in filters and filters['option_types']:
            filtered = [
                c for c in filtered
                if c.get('option_type') in filters['option_types']
            ]
        ...
        if 'liquidity_min' in filters:
            filtered = [
                c for c in filtered
                if (c.get('oi', 0) + c.get('volume', 0)) >= filters['liquidity_min']
            ]
    ...
    def filter_positions(
        ...
        if 'underlyings' in filters and filters['underlyings']:
            filtered = [
                p for p in filtered
                if p.get('underlying') in filters['underlyings']
            ]
        ...
        if 'pnl_range' in filters:
            ...
            filtered = [
                p for p in filtered
                if min_pnl <= p.get('unrealized_pnl', 0) <= max_pnl
            ]
        ...
        if 'entry_date_range' in filters:
            ...
            if min_date or max_date:
                filtered = [
                    p for p in filtered
                    if self._date_in_range(p.get('entry_time'), min_date, max_date)
                ]
        ...
        if 'strategies' in filters and filters['strategies']:
            filtered = [
                p for p in filtered
                if p.get('strategy') in filters['strategies']
            ]
    ...
    def filter_signals(
        ...
        if 'confidence_min' in filters:
            filtered = [
                s for s in filtered
                if s.get('confidence', 0) >= filters['confidence_min']
            ]
        ...
        if 'underlyings' in filters and filters['underlyings']:
            filtered = [
                s for s in filtered
                if s.get('underlying') in filters['underlyings']
            ]
        ...
        if 'strategy_types' in filters and filters['strategy_types']:
            filtered = [
                s for s in filtered
                if s.get('strategy') in filters['strategy_types']
            ]
        ...
        if 'actions' in filters and filters['actions']:
            filtered = [
                s for s in filtered
                if s.get('action') in filters['actions']
            ]
    ...
    def get_filter_presets(self, filter_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if filter_type:
            return [
                v for k, v in self.saved_filters.items()
