...
class MultiValidationAudit:
    ...
    def validate_spot_price(
        ...
        consensus_all = sum([v["price"] for v in validations if v.get("price")]) / len([v for v in validations if v.get("price")]) if validations else None
        ...
        result = {
            ...
            "consensus": {
                "online": consensus_online,
                "offline": consensus_offline,
                "all": consensus_all
            },
            "status": status,
    ...
    def validate_pnl(
        ...
        diff = abs(reported_pnl - internal_pnl) / abs(internal_pnl) if internal_pnl != 0 else abs(reported_pnl - internal_pnl)
        validations.append({
            "source": "internal_calc",
            "pnl": internal_pnl,
    ...
    def comprehensive_audit(
        ...
        if chain_data.get('spot'):
            spot_validation = self.validate_spot_price(
                chain_data.get('underlying', 'NIFTY'),
                chain_data.get('spot', 0)
            )
        ...
        for position in positions:
            pnl_validation = self.validate_pnl(
                position,
