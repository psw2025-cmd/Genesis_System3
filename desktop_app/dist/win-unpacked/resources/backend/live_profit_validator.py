...
class LiveProfitValidator:
    ...
    def validate_against_market_data(
        ...
        try:
            ...
            option_price = self._calculate_option_price(
                spot_price, strike, option_type, expiry
            )
    ...
    def _find_similar_positions(
        ...
        for hist_pos in historical_data:
            if (hist_pos.get('underlying', '') == underlying and
                hist_pos.get('option_type', '') == option_type and
                abs(hist_pos.get('strike', 0) - position.get('strike', 0)) <= strike_range):
