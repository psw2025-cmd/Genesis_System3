...
def test_forward_signal_integrity() -> dict:
    ...
    test2 = {
        "name": "ts column is valid datetime",
        "passed": False,
        "details": ""
    }
    ...
    if "ts" not in df.columns:
        ...
        if ts_null_count == len(df):
            ...
            try:
                ...
                test2["details"] = f"✅ {valid_ts}/{len(df)} valid timestamps ({100*valid_ts/len(df):.1f}%)" if test2["passed"] else f"⚠️ Only {valid_ts}/{len(df)} valid timestamps"
            except Exception as e:
    ...
    test3 = {
        "name": "side column is valid (BUY/SELL/HOLD)",
        "passed": False,
        "details": ""
    }
    ...
    test4 = {
        "name": "Key columns integrity (underlying, strike, expiry)",
        "passed": False,
        "details": ""
    }
    ...
    test5 = {
        "name": "Forward return columns exist and populated",
        "passed": False,
        "details": ""
    }
