...
class TestPhase117(unittest.TestCase):
    """Test suite for Phase 117."""
    
    @classmethod
    def setUpClass(cls):
        """Set up test class."""
        cls.phase_num = 117
        cls.module_name = "core.engine.system3_phase117_live_to_learning_bridge"
        cls.func_name = "run_phase117"
    ...
    def test_phase_outputs(self):
        ...
        try:
            ...
            # Check for storage/live/phase114_live_session_health.md
            if "storage/live/phase114_live_session_health.md" in str(result.get("outputs", {})):
                output_path = PROJECT_ROOT / "storage/live/phase114_live_session_health.md"
            ...
            # Check for logs/phase112_session_loop.log
            if "logs/phase112_session_loop.log" in str(result.get("outputs", {})):
                output_path = PROJECT_ROOT / "logs/phase112_session_loop.log"
                # Note: File may not exist if phase hasn't run yet
                # self.assertTrue(output_path.exists(), f"Expected output file {output_path} not found")
            # Check for logs/phase106_dryrun_execution.log
            if "logs/phase106_dryrun_execution.log" in str(result.get("outputs", {})):
                output_path = PROJECT_ROOT / "logs/phase106_dryrun_execution.log"
                # Note: File may not exist if phase hasn't run yet
                # self.assertTrue(output_path.exists(), f"Expected output file {output_path} not found")
            # Check for storage/live/live_orders_ledger.csv
            if "storage/live/live_orders_ledger.csv" in str(result.get("outputs", {})):
                output_path = PROJECT_ROOT / "storage/live/live_orders_ledger.csv"
                # Note: File may not exist if phase hasn't run yet
                # self.assertTrue(output_path.exists(), f"Expected output file {output_path} not found")
            # Check for storage/live/phase101_live_trade_config_snapshot.json
            if "storage/live/phase101_live_trade_config_snapshot.json" in str(result.get("outputs", {})):
                output_path = PROJECT_ROOT / "storage/live/phase101_live_trade_config_snapshot.json"
                # Note: File may not exist if phase hasn't run yet
                # self.assertTrue(output_path.exists(), f"Expected output file {output_path} not found")
