#!/usr/bin/env python3
import json, os, subprocess, time, zipfile, urllib.request, urllib.error
...
if local_head != origin_head:
    add_issue("HIGH", "repo_sync", "Local HEAD does not match origin/main", f"local={local_head}, origin={origin_head}", "Sync before using any proof.")
...
for f in compile_targets:
    ...
    if r["rc"] != 0:
        add_issue("HIGH", "compile", f"Python compile failed: {f}", r["stderr"][:500], "Fix syntax/import compile error.")
...
for r in endpoint_results:
    if not r["ok"] or r.get("http") != 200:
        add_issue("HIGH", "cloud_endpoint", f"Endpoint failed: {r['url']}", f"http={r.get('http')} error={r.get('error')}", "Fix Render backend route/deploy.")
...
if broker:
    if broker.get("connected") is not True:
        add_issue("CRITICAL","broker","Dhan broker is not connected",f"connected={broker.get('connected')} error={broker.get('error')} credentials_present={broker.get('credentials_present')}","Fix Render DHAN_ACCESS_TOKEN / DHAN_PIN / DHAN_TOTP_SECRET, then verify connected=true.")
    if broker.get("error"):
        add_issue("HIGH","broker","Broker endpoint reports error",str(broker.get("error")),"Refresh/regenerate token and re-run proof.")
    if broker.get("live_trading_enabled") is True or broker.get("order_placement_allowed") is True:
        add_issue("CRITICAL","live_safety","Live/order placement is enabled unexpectedly",json.dumps(broker)[:500],"Immediately disable LIVE_TRADING_ENABLED and SYSTEM3_LIVE_TRADING_ALLOWED.")
else:
...
if health:
    if health.get("live_allowed") is True:
        add_issue("CRITICAL","live_safety","/api/health says live_allowed=true",json.dumps(health)[:500],"Disable live immediately.")
    if health.get("status") not in ("ok","ready","running"):
        add_issue("HIGH","system_health","/api/health is not ready",f"status={health.get('status')} message={health.get('message')} blockers={health.get('live_blockers')}","Fix broker/data health before market proof.")

if state:
    if str(state.get("mode","")).upper() == "LIVE":
        add_issue("CRITICAL","live_safety","/api/state mode is LIVE",json.dumps(state)[:500],"Disable live mode immediately.")
    if str(state.get("data_source","")).upper() in ("SYNTHETIC","NOT_READY",""):
        add_issue("HIGH","data","Dashboard state is not using proven real broker data",f"data_source={state.get('data_source')}","Run broker-connected real data proof.")
    broker_state = state.get("broker") or {}
    if broker_state and broker_state.get("connected") is not True:
        add_issue("CRITICAL","broker","State broker is disconnected",f"error={broker_state.get('error')}","Fix Dhan token/session.")
...
if proof_matrix:
    if proof_matrix.get("trade_ready") is not True:
        add_issue("HIGH","readiness","Proof matrix trade_ready is false",f"verdict={proof_matrix.get('verdict')}","Clear all proof warnings before live readiness.")
    for row in proof_matrix.get("rows",[]):
        status = row.get("status")
        if status in ("FAIL","PASS_WITH_WARNINGS"):
            sev = "CRITICAL" if status == "FAIL" else "MEDIUM"
            add_issue(sev,"proof_gate",f"{row.get('name')} is {status}",f"warnings={row.get('warnings')} blockers={row.get('blockers')}","Resolve or produce proof.")

if master_pipeline:
    if master_pipeline.get("trade_ready") is not True:
        add_issue("HIGH","pipeline","Full trading pipeline is not trade ready",f"blockers={master_pipeline.get('blockers')} verdict={master_pipeline.get('verdict')}","Run fresh training, costed backtest, market paper trade, dashboard proof.")

if fresh_data:
    ev = fresh_data.get("evidence",{})
    if ev.get("fresh_broker_live_data_proven") is not True:
        add_issue("HIGH","data","Fresh broker live data is not proven",fresh_data.get("next_action",""),"Run secure broker data proof in cloud with valid Dhan token.")

if model_proof:
    ev = model_proof.get("evidence",{})
    if ev.get("fresh_training_metrics_proven") is not True:
        add_issue("MEDIUM","model","Fresh training metrics not proven","","Run dry-run model load/training proof with metrics.")
...
if backtest_proof:
    ...
    if ev.get("recent_costed_walkforward_proven") is not True:
        add_issue("HIGH","backtest","Recent costed walk-forward not proven","","Run recent walk-forward with brokerage/slippage/spread/liquidity.")
...
if lifecycle_proof:
    ...
    if ev.get("full_lifecycle_proven") is not True or ev.get("orders_trades_lifecycle_reconciled") is not True:
        add_issue("HIGH","paper_lifecycle","Signal→order→fill→exit→P&L lifecycle not proven/reconciled",f"mandatory_fields={ev.get('mandatory_lifecycle_fields')}","Run market-day analyzer paper lifecycle proof.")
...
if dashboard_truth:
    ...
    if ev.get("api_db_report_reconciliation_proven") is not True:
        add_issue("MEDIUM","dashboard","API/DB/report reconciliation not proven","","Run dashboard truth reconciliation.")
...
_LIVE_FLAG_SKIP_PATTERNS = [
    "test_suite", "_check.py", "# ", '"LIVE_TRADING', "'LIVE_TRADING",
    'warnings.append', '__pycache__', '.bak',
]
live_flag_lines = []
for _search_path in ["config", "core", "scripts", "dashboard", ".github", "render.yaml"]:
    ...
    for _f in _files:
        if not _f.is_file(): continue
        try:
            for _i, _line in enumerate(_f.read_text(errors="replace").splitlines(), 1):
                _ls = _line.strip()
                if not _ls or _ls.startswith("#"): continue
                if any(p in str(_f) or p in _ls for p in _LIVE_FLAG_SKIP_PATTERNS): continue
                import re as _re
                if _re.search(r'^\s*LIVE_TRADING_ENABLED\s*=\s*True|^\s*USE_LIVE_EXECUTION_ENGINE\s*=\s*True|^\s*SYSTEM3_LIVE_TRADING_ALLOWED\s*=\s*["\']?1["\']?', _line):
                    live_flag_lines.append(f"{_f.relative_to(ROOT)}:{_i}: {_ls[:100]}")
        except Exception: pass
if live_flag_lines:
    add_issue("CRITICAL","live_safety","Truthy live flag found in source (real assignment)","\n".join(live_flag_lines[:20]),"Disable live flags immediately.")
...
if wrapper_path.exists():
    ...
    if "NOT_IMPLEMENTED" in wrapper_text or "skeleton" in wrapper_text.lower():
        add_issue("LOW","live_execution","Live order wrapper is placeholder — live trading intentionally blocked","core/broker/dhan_live_order_wrapper.py","SAFETY FEATURE: keeps live trading impossible until explicitly implemented. Do not implement until all proof gates pass.")

render_yaml = (ROOT / "render.yaml").read_text(encoding="utf-8", errors="replace") if (ROOT / "render.yaml").exists() else ""
if "type: worker" not in render_yaml or "scripts/cloud_worker.py" not in render_yaml:
    ...
    add_issue("MEDIUM","cloud_worker","Worker runtime logs not proven by API endpoint check","render.yaml has worker, but this command cannot read Render service logs","Capture Render worker logs showing token-daemon/watchdog/scheduler started.")
...
md.append("|---|---|---|---|---|")
for i in ISSUES:
    md.append(f"| {i['severity']} | {i['area']} | {i['issue']} | {str(i['proof']).replace('|','/')[:300]} | {str(i['action']).replace('|','/')[:300]} |")
