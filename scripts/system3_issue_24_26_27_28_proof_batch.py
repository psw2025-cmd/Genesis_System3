...
def read(path: str, n=300000) -> str:
    try:
        return (ROOT / path).read_text(encoding="utf-8", errors="replace")[:n]
    except Exception:
        return ""
    ...
    for f in all_files:
        ...
        if p.suffix.lower() in TEXT_EXT and p.exists() and p.stat().st_size < 2_000_000:
            ...
            if any(t in txt for t in terms):
                hit.append("content")
        if hit:
            out.append({"file": f, "hit": "+".join(hit)})
    return out[:300]
...
def repo_authority(all_files):
    ...
    for f in all_files:
        ...
        cls = (
            ...
            if f.startswith("reports/")
                ...
                "NEEDS_HUMAN_REVIEW"
                if any(x in f.lower() for x in ["old", "backup", "copy", "archive", "bak"])
                else "KEEP_ACTIVE"
            )
        )
        inv.append(
        ...
        if p.exists() and p.suffix.lower() in TEXT_EXT and p.stat().st_size < 5_000_000:
            try:
                hashes.append({"path": f, "sha256": sha256(p)})
            except Exception as e:
                hashes.append({"path": f, "sha256": "", "error": repr(e)})
    ...
    write_json(d / "cleanup_summary.json", summ)
    archive(d, "repo_authority")
    return status(
        "repo_authority",
        "#28",
        st,
        "Repo authority proof pack created. No deletion performed.",
        [rel(x) for x in d.iterdir()],
        warnings=["deep dependency trace still needs enhancement"],
    )


def benchmark(all_files):
    d = LATEST / "performance_benchmark"
    d.mkdir(parents=True, exist_ok=True)
    ...
    blocked = []
    if not pred:
        blocked.append("prediction_source_not_found")
    if not actual:
        blocked.append("actual_market_result_source_not_found")
    st = "BLOCKED" if blocked else "PARTIAL"
    write_csv(
        d / "prediction_vs_actual.csv",
        [],
        ["symbol", "prediction_time", "predicted_rank", "actual_rank", "direction_ok", "notes"],
    )
    write_csv(d / "top_mover_match.csv", [], ["symbol", "predicted_top", "actual_top", "match", "notes"])
    write_text(
        d / "missed_opportunities.md", "# Missed Opportunities\n\nNo computed rows yet. Source discovery required.\n"
    )
    ...
    write_json(
        d / "status.json",
        {
            "status": st,
            "blockers": blocked,
            "prediction_candidate_count": len(pred),
            "actual_candidate_count": len(actual),
        },
    )
    write_text(
        d / "benchmark_summary.md",
    ...
    return status(
        ...
        "#26",
        st,
        "Benchmark source discovery completed; no fake metrics generated.",
        [rel(x) for x in d.iterdir()],
        blocked,
    )
...
def lifecycle(all_files):
    d = LATEST / "lifecycle_reconciliation"
    d.mkdir(parents=True, exist_ok=True)
    ...
    write_json(
        d / "status.json",
        {"status": st, "blockers": blockers, "candidate_count": len(cand), "analyzer_paper_only": True},
    )
    write_text(
        d / "lifecycle_summary.md",
        f"# Lifecycle Reconciliation\n\nStatus: {st}\n\nAnalyzer/Paper only. No live action.\n",
    )
    ...
    return status(
        ...
        "#27",
        st,
        "Lifecycle source discovery completed; no inferred fills used.",
        [rel(x) for x in d.iterdir()],
        blockers,
    )
...
def dashboard(all_files):
    d = LATEST / "dashboard_browser_api_proof"
    d.mkdir(parents=True, exist_ok=True)
    ...
    if localhost:
        blockers.append("localhost_references_present")
    if not backtest:
        blockers.append("backtest_ui_reference_not_found")
    write_text(
        d / "localhost_reference_scan.txt",
        "\n".join(localhost) if localhost else "No localhost references found in frontend scan.\n",
    )
    write_text(
        d / "backtest_ui_proof.md",
        "# Backtest UI Proof\n\n"
        + ("\n".join(f"- `{x}`" for x in backtest) if backtest else "Backtest UI reference not found."),
    )
    write_json(
        d / "api_smoke_results.json",
        {
            "status": "BLOCKED",
            "reason": "local backend/frontend browser run not executed by static script",
            "api_candidates": api_files[:80],
        },
    )
    write_json(
        d / "screenshot_manifest.json",
        {"status": "BLOCKED", "screenshots": [], "reason": "browser screenshot capture requires local/browser runtime"},
    )
    write_text(
        d / "frontend_build.log",
        "Frontend build not run by this static GitHub-created proof script. Run locally for full proof.\n",
    )
    write_text(
        d / "npm_audit_summary.md",
    ...
    write_json(
        d / "status.json",
        {
            ...
            "backtest_reference_count": len(backtest),
        },
    )
    write_text(
        d / "dashboard_browser_api_summary.md",
        f"# Dashboard Browser/API Proof\n\nStatus: {st}\n\nBlockers: {blockers}\n",
    )
    archive(d, "dashboard_browser_api_proof")
    return status(
        "dashboard_browser_api",
        "#24",
        st,
        "Static dashboard scan completed; browser/API proof remains blocked until local run.",
        [rel(x) for x in d.iterdir()],
        blockers,
    )
...
def main():
    all_files = files()
    batch = LATEST / "implementation_batch"
    ...
    statuses.append(
        status(
            ...
            [rel(batch / "search_findings.md"), rel(batch / "search_findings.json")],
        )
    )
    ...
    write_json(
        batch / "status_matrix.json", {"generated_utc": datetime.now(timezone.utc).isoformat(), "statuses": statuses}
    )
