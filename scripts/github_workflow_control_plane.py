...
The report is intentionally metadata-only. It never reads secret values, never
mutates GitHub, and separates current-main failures from superseded history.
"""
...
def _safe_run(run: dict[str, Any], current_main_sha: str) -> dict[str, Any]:
    return {
        ...
        "scope": run_scope(run, current_main_sha),
    ...
    def get(self, path: str) -> dict[str, Any] | list[Any]:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "genesis-system3-workflow-control-plane",
            },
            method="GET",
        )
        try:
...
def build_inventory(client: GitHubReadOnly, repository: str) -> dict[str, Any]:
    ...
    issues_payload = client.get(
        f"/repos/{owner}/{repo}/issues?state=open&sort=updated&direction=desc&per_page=100"
    )
        ...
        workflow_inventory.append(
            {
                ...
                "html_url": _safe_url(workflow.get("html_url")),
    ...
    recent_failures = []
    for raw, safe in zip(raw_runs, safe_runs):
        created = _iso_to_dt(raw.get("created_at"))
        if created and created >= cutoff and is_terminal_failure(raw.get("conclusion")):
            recent_failures.append(safe)

    run_evidence = []
    # Current-main runs are the authority. Bound detailed API calls to the latest 30.
    for run in current_runs[:30]:
        ...
        run_evidence.append(
            {
                "run": run,
                "failed_jobs": summarize_failed_steps(jobs_payload),
    ...
    if isinstance(issues_payload, list):
        open_items = [_safe_issue(item) for item in issues_payload]

    report = {
        "schema_version": 1,
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "repository": repository,
        "source_authority": "github-api-read-only",
        "current_main_sha": current_main_sha,
        "workflow_count": len(workflow_inventory),
        "workflows": workflow_inventory,
        "current_main_runs": current_runs,
        "current_main_failures": current_failures,
        "active_runs": active_runs,
        "recent_48h_failures": recent_failures,
        "current_main_run_evidence": run_evidence,
        "open_issues_and_prs": open_items,
        "counts": {
            "current_main_runs": len(current_runs),
            "current_main_failures": len(current_failures),
            "active_runs": len(active_runs),
            "recent_48h_failures": len(recent_failures),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# System3 Workflow Control Plane",
        "",
        f"- Generated UTC: `{report['generated_at_utc']}`",
        f"- Current main SHA: `{report['current_main_sha']}`",
        f"- Workflows inventoried: **{report['workflow_count']}**",
        f"- Current-main workflow runs: **{counts['current_main_runs']}**",
        f"- Current-main failures: **{counts['current_main_failures']}**",
        f"- Active runs: **{counts['active_runs']}**",
        f"- Failures seen in last 48h: **{counts['recent_48h_failures']}**",
        f"- Current-main artifacts indexed: **{counts['current_main_artifacts']}**",
        f"- Open issues / PRs: **{counts['open_issues']} / {counts['open_prs']}**",
        "",
    ...
    if not failures:
        ...
        for run in failures:
            lines.append(
                f"- `{run['name']}` run `{run['id']}` — **{run['conclusion']}** — {run.get('html_url') or 'no-url'}"
            )
    lines += ["", "## Active runs"]
    ...
    if not active:
        ...
        for run in active:
            lines.append(
                f"- `{run['name']}` run `{run['id']}` — `{run['status']}` — `{run['scope']}`"
            )
    lines += [
        "",
        "## Safety",
        "- Read-only GitHub API inventory: **true**",
        "- GitHub/runtime mutation performed: **false**",
        "- Secret values accessed: **false**",
        "- Order actions performed: **false**",
...
def _self_test() -> int:
    assert is_terminal_failure("failure")
    assert not is_terminal_failure("success")
    sha = "a" * 40
...
def main() -> int:
    ...
    (OUT_DIR / "workflow_inventory.json").write_text(
