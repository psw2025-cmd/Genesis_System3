#!/usr/bin/env python3
"""Create sanitized, read-only Genesis System3 Google Cloud runtime evidence.

Evidence is bound to the immutable revision receiving 100% production traffic.
A latest-created/latest-ready or tagged zero-traffic candidate is never accepted
as proof of what users are actually seeing.
"""
...
SAFE_ENV = {
    ...
    "SYSTEM3_LIVE_TRADING_ALLOWED", "AUTO_EXECUTE_TRADES",
    "SYSTEM3_STATE_BACKEND", "SYSTEM3_STATE_BACKEND_REQUIRED",
    "SYSTEM3_REAL_ONLY", "CLOUD_PAPER_ENGINE", "DEFER_INSTRUMENT_WARMUP",
...
# Retired dashboard credential/session names are evidence-only deny markers. They
# are not supported configuration. Presence on the serving revision is a hard
# runtime blocker even when a value appears disabled.
...
SECRET_BACKED_ENV_NAMES = set(RETIRED_DASHBOARD_SECRET_ENV) | {"WORKER_PUSH_TOKEN"}
REDACT = re.compile(
...
def container(resource: dict[str, Any]) -> dict[str, Any]:
    ...
    if not rows:
        rows = (resource.get("spec") or {}).get("containers") or []
    if not rows:
        rows = (resource.get("template") or {}).get("containers") or []
    return rows[0] if rows else {}


def service_account(resource: dict[str, Any]) -> str | None:
    return (
        ((((resource.get("spec") or {}).get("template") or {}).get("spec") or {}).get("serviceAccountName"))
        or (resource.get("spec") or {}).get("serviceAccountName")
        or ((resource.get("spec") or {}).get("template") or {}).get("serviceAccount")
        or resource.get("serviceAccount")
    )


def serving_traffic(service: dict[str, Any]) -> tuple[str, dict[str, int], list[dict[str, Any]]]:
    ...
    serving = ""
    if len(allocations) == 1 and list(allocations.values()) == [100]:
        serving = next(iter(allocations))
    return serving, dict(sorted(allocations.items())), raw


def safe_env(resource: dict[str, Any]) -> tuple[dict[str, Any], list[str], list[str], list[str]]:
    env: dict[str, Any] = {}
    secret_refs: list[str] = []
    plaintext_secret_names: list[str] = []
    all_names: list[str] = []
    for row in container(resource).get("env") or []:
        name = str(row.get("name") or "")
        if not name:
            continue
        all_names.append(name)
        has_secret_ref = bool(
            (row.get("valueFrom") or {}).get("secretKeyRef")
            or (row.get("valueSource") or {}).get("secretKeyRef")
        )
        if has_secret_ref:
            secret_refs.append(name)
        elif name in SAFE_ENV or name in RETIRED_DASHBOARD_ENV:
            env[name] = row.get("value")
        elif name in SECRET_BACKED_ENV_NAMES:
            plaintext_secret_names.append(name)
    return env, sorted(set(secret_refs)), sorted(set(plaintext_secret_names)), sorted(set(all_names))
...
def evaluate_safety(
    ...
    """Evaluate permanent public-readonly + LIVE-off serving posture."""
    retired_env_present = sorted(RETIRED_DASHBOARD_ENV & set(all_env_names))
    retired_secret_mounts = sorted(RETIRED_DASHBOARD_SECRET_ENV & set(secret_refs))
    retired_plaintext = sorted(RETIRED_DASHBOARD_SECRET_ENV & set(plaintext_secret_names))
    credential_surface_absent = not (
        retired_env_present or retired_secret_mounts or retired_plaintext
    )
    return {
        "analyze_mode": is_on(env.get("ANALYZE_MODE")),
        ...
        # Compatibility field names retained for downstream report consumers;
        # they now mean actual detected drift, not desired false configuration.
        "api_key_required": any(name.startswith("REQUIRE_") for name in retired_env_present),
        ...
        "retired_dashboard_plaintext": retired_plaintext,
        "dashboard_credential_surface_absent": credential_surface_absent,
        "dashboard_public_readonly": credential_surface_absent,
...
def safety_passes(safety: dict[str, Any]) -> bool:
    return (
        ...
        and safety["dashboard_credential_surface_absent"]
...
def safety_blockers(safety: dict[str, Any]) -> list[str]:
    ...
    if safety["retired_dashboard_env_present"]:
        blockers.append(
            "Retired dashboard credential/session environment configuration is present on the serving revision."
        )
    if safety["retired_dashboard_secret_mounts"]:
        blockers.append("Retired dashboard credential secret mount is present on the serving revision.")
    if safety["retired_dashboard_plaintext"]:
        blockers.append("Retired dashboard credential is present as plaintext runtime configuration.")
    return blockers
...
def log_summary(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        checks = {
            ...
            "out_of_memory": ("out of memory", "memory limit exceeded", "memory limit of", "killed process"),
    ...
    return {
        ...
        "p50_latency_ms": quantile(latencies, .50),
        "p95_latency_ms": quantile(latencies, .95),
        "p99_latency_ms": quantile(latencies, .99),
        "latest_successful_health_request": latest_health,
...
def metric(metric_type: str, start: str, end: str) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "series_count": len(data.get("timeSeries") or []),
...
def endpoint_summary(path: str, body: Any) -> dict[str, Any]:
    ...
    if path == "/api/auth/status":
        return {k: body.get(k) for k in (
            "required", "configured", "authenticated", "mode", "credential_surface"
        )}
    if path == "/api/health":
    ...
    if path == "/api/broker/status":
        ...
        return {
            ...
            "error_present": bool(body.get("error")),
            "token_source": proof.get("source"),
            "secret_version": proof.get("secret_version"),
            "loaded_at_utc": proof.get("loaded_at_utc"),
    ...
    if path.startswith("/api/chain/"):
        ...
        return {
            "symbol": body.get("symbol") or path.rsplit("/", 1)[-1],
    ...
    if path in {"/api/pnl", "/api/trades/today", "/api/paper"}:
        return {
            "status": body.get("status"),
            "data_source": body.get("data_source") or body.get("source"),
    ...
    if path == "/api/auto_gates":
        ...
        return {
            "status": body.get("status") or body.get("verdict"),
            "production_grade_claim_allowed": body.get("production_grade_claim_allowed"),
            "trade_ready": body.get("trade_ready"),
    ...
    if path == "/api/ml/performance":
        return {
            "status": body.get("status"),
            "days_recorded": body.get("days_recorded") or body.get("validation_days"),
...
def fetch_endpoint(base: str, path: str) -> dict[str, Any]:
    req = urllib.request.Request(
        base.rstrip("/") + path,
        headers={"Accept": "application/json", "User-Agent": "System3Evidence/2.0"},
    )
    started = now()
    status = None
    raw = b""
    ...
    return {
        ...
        "json_valid": body is not None,
        "summary": endpoint_summary(path, body) if body is not None else {},
        "error": error,
...
def secret_metadata(name: str) -> dict[str, Any]:
    secret, describe = run_json([
        "gcloud", "secrets", "describe", name, f"--project={PROJECT}", "--format=json"
    ])
    versions, query = run_json([
...
def markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Genesis_System3 GCP Runtime Lock",
        "",
        f"- Generated: `{report['generated_at_utc']}`",
        f"- Project: `{PROJECT}`",
        f"- Service: `{SERVICE}`",
        f"- Serving revision: `{report['cloud_run'].get('serving_revision')}`",
        f"- Repository SHA: `{report.get('repository_commit') or 'UNKNOWN'}`",
        f"- Serving SHA: `{report.get('deployed_commit') or 'UNKNOWN'}`",
        f"- Verdict: **{report['lock_result']}**",
        "",
        "## Safety",
        "",
    ]
...
def collect() -> dict[str, Any]:
    end_dt = now()
    start = iso(end_dt - dt.timedelta(hours=24))
    end = iso(end_dt)
    commands: dict[str, Any] = {}

    service, commands["service"] = run_json([
        "gcloud", "run", "services", "describe", SERVICE,
        f"--project={PROJECT}", f"--region={REGION}", "--format=json",
    ])
    service = service if isinstance(service, dict) else {}
    serving_revision, traffic_allocations, raw_traffic = serving_traffic(service)
    latest_ready = str(((service.get("status") or {}).get("latestReadyRevisionName")) or "")
    latest_created = str(((service.get("status") or {}).get("latestCreatedRevisionName")) or "")

    serving_resource: dict[str, Any] = {}
    commands["serving_revision"] = {"ok": False, "error": "serving_revision_unavailable"}
    if serving_revision:
        serving_resource, commands["serving_revision"] = run_json([
            "gcloud", "run", "revisions", "describe", serving_revision,
            f"--project={PROJECT}", f"--region={REGION}", "--format=json",
        ])
        serving_resource = serving_resource if isinstance(serving_resource, dict) else {}

    env, secret_refs, plaintext_secret_names, all_env_names = safe_env(serving_resource)
    c0 = container(serving_resource)
    url = ((service.get("status") or {}).get("url")) or service.get("uri") or ""

    iam, commands["iam"] = run_json([
    ...
    logs, commands["logs"] = run_json([
        "gcloud", "logging", "read", flt, f"--project={PROJECT}",
        "--limit=500", "--order=desc", "--format=json",
    ], 120)

    paths = [
        "/api/auth/status", "/api/health", "/api/state", "/api/broker/status",
    ...
    safety = evaluate_safety(env, secret_refs, plaintext_secret_names, all_env_names)
    safety_pass = safety_passes(safety)
    broker = endpoints.get("/api/broker/status", {}).get("summary", {})
    broker_ready = bool(
        broker.get("connected")
        and broker.get("token_source") == "GCP_SECRET_MANAGER_DYNAMIC"
    ...
    chains_ready = all(
        v.get("ok")
        and not v.get("summary", {}).get("stale")
        and int(v.get("summary", {}).get("contract_count") or 0) > 0
        for v in chains.values()
    )
    health_ok = bool(endpoints.get("/api/health", {}).get("ok"))
    auth_summary = endpoints.get("/api/auth/status", {}).get("summary", {})
    public_auth_contract = bool(
        auth_summary.get("required") is False
        and auth_summary.get("configured") is False
        and auth_summary.get("authenticated") is False
        and auth_summary.get("mode") == "public_readonly"
        and auth_summary.get("credential_surface") == "REMOVED"
    )

    blockers: list[str] = []
    if not commands["service"]["ok"]:
        blockers.append("Cloud Run service could not be read.")
    if not serving_revision:
        blockers.append("A single exact 100%-traffic serving Cloud Run revision is not proven.")
    if not commands["serving_revision"]["ok"]:
        blockers.append("Serving Cloud Run revision metadata could not be read.")
    if not source_match:
        blockers.append("GitHub SHA and 100%-traffic serving DEPLOY_GIT_SHA are missing or different.")
    blockers += safety_blockers(safety)
    if not public_auth_contract:
        blockers.append("Permanent public-readonly dashboard status contract is not proven.")
    if not health_ok:
        blockers.append("Public /api/health proof failed.")
    if not broker_ready:
        blockers.append("Dhan read-only broker proof is not ready.")
    if not chains_ready:
        blockers.append("All four required option chains are not fresh and populated.")
    if not commands["scheduler"]["ok"]:
        blockers.append("Token scheduler metadata is unavailable.")

    deployment_locked = bool(
        serving_revision and source_match and safety_pass and public_auth_contract and health_ok
    )
    result = (
        "ANALYZER_LOCKED" if deployment_locked and broker_ready and chains_ready
        else "DEPLOYMENT_LOCKED" if deployment_locked
    ...
    return {
        "schema_version": 2,
        "generated_at_utc": iso(end_dt),
        "project_id": PROJECT,
        "region": REGION,
        "service": SERVICE,
        "repository_commit": EXPECTED_SHA or None,
        "deployed_commit": deployed_sha or None,
        "source_matches_deployment": source_match,
        "cloud_run": {
            "serving_revision": serving_revision or None,
            "latest_ready_revision": latest_ready or None,
            "latest_created_revision": latest_created or None,
            "traffic_allocations": traffic_allocations,
            "traffic_entries": raw_traffic,
            "service_url": url or None,
            "image": c0.get("image"),
            "service_account": service_account(serving_resource),
            "resources": c0.get("resources") or {},
            "scaling": template.get("scaling") or {},
            "public_invoker": public,
            "safe_environment": env,
            "all_environment_names": all_env_names,
            "mounted_secret_environment_names": secret_refs,
            "plaintext_secret_environment_names_detected": plaintext_secret_names,
        },
        "safety": safety,
        "scheduler": {
            "name": (scheduler or {}).get("name") if isinstance(scheduler, dict) else None,
        ...
        "monitoring_24h": {
            "request_count": metric("run.googleapis.com/request_count", start, end),
        ...
        "all_required_chains_ready": chains_ready,
        "commands": commands,
        "blockers": blockers,
        "lock_result": result,
...
def main() -> int:
    ...
    report = collect()
    (OUT / "gcp_runtime_lock.json").write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (OUT / "gcp_runtime_lock.md").write_text(markdown(report), encoding="utf-8")
    print(json.dumps({
        "report": str(OUT / "gcp_runtime_lock.json"),
        "lock_result": report["lock_result"],
        "source_matches_deployment": report["source_matches_deployment"],
        "serving_revision": report["cloud_run"].get("serving_revision"),
