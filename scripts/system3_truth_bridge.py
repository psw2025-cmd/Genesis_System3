...
def analyze_live(live: dict) -> list[dict]:
    ...
    for name, ep in live.items():
        if not ep.get("ok"):
            add_issue(
                issues,
                "HIGH",
                "LIVE_ENDPOINT_FAILED",
                f"Live endpoint failed: {name}",
                {"endpoint": name, "error": ep.get("error"), "status_code": ep.get("status_code")},
                "Check Render endpoint/service logs.",
            )
    ...
    if state_broker_connected is True and s(health_broker_status) not in ("connected", "ok"):
        add_issue(
            issues,
            "HIGH",
            "BROKER_STATE_HEALTH_MISMATCH",
            "State says broker connected but health does not.",
            {"state_broker_connected": state_broker_connected, "health_broker_status": health_broker_status},
        )
    if direct_broker_connected is True and state_broker_connected is False:
        add_issue(
            issues,
            "HIGH",
            "BROKER_DIRECT_STATE_MISMATCH",
            "Direct broker endpoint connected but state not connected.",
            {"direct_broker_connected": direct_broker_connected, "state_broker_connected": state_broker_connected},
        )
    if state_broker_connected is True and "BROKER_DISCONNECTED" in alert_codes:
        add_issue(
            issues,
            "HIGH",
            "FALSE_BROKER_DISCONNECTED",
            "False broker disconnected alert active while broker is connected.",
    ...
    if s(state_ds) == "synthetic" and state_broker_connected is True and market_open is False:
        add_issue(
            issues,
            "MEDIUM",
            "BAD_CLOSED_MARKET_DATA_SOURCE",
            "Closed market + connected broker should not be labelled SYNTHETIC.",
            {"state_data_source": state_ds},
        )
    if state_ds and health_ds and s(state_ds) != s(health_ds):
        ...
        if (s(state_ds), s(health_ds)) not in allowed:
            add_issue(
                ...
                "State and health data_source labels differ.",
                {"state_data_source": state_ds, "health_data_source": health_ds},
            )
    if (
        s(state.get("mode")) == "live"
        or broker_state.get("live_trading_enabled") is True
        or broker_state.get("order_placement_allowed") is True
    ):
        add_issue(
            issues,
            "CRITICAL",
            "LIVE_SAFETY_BREACH",
            "Live mode/order placement appears enabled.",
            {
                "mode": state.get("mode"),
                "live_trading_enabled": broker_state.get("live_trading_enabled"),
                "order_placement_allowed": broker_state.get("order_placement_allowed"),
            },
        )
    ...
    for name, ep in live.items():
        ...
        if state_broker_connected is True and s(data.get("status")) == "not_ready":
            add_issue(
                issues,
                "HIGH",
                "CHAIN_NOT_READY_WITH_BROKER",
                f"{name} says NOT_READY while broker is connected.",
                {"chain_status": data.get("status"), "message": data.get("message")},
            )
        if market_open is True and int(total or 0) == 0:
            add_issue(
                issues,
                "HIGH",
                "ZERO_CHAIN_CONTRACTS_MARKET_OPEN",
                f"{name} has zero contracts while market is open.",
...
def analyze_proofs(proofs: dict) -> list[dict]:
    ...
    if fpr and fpr.get("trade_ready") is not True:
        add_issue(
            ...
            {"verdict": fpr.get("verdict"), "blockers": fpr.get("blockers")},
            "Run real market analyzer paper lifecycle and fresh broker data proof.",
        )
    ...
    if matrix and matrix.get("trade_ready") is not True:
        add_issue(
            issues,
            "HIGH",
            "PROOF_MATRIX_INCOMPLETE",
            "Proof matrix still says not trade ready.",
    ...
    for row in matrix.get("rows", []) if isinstance(matrix.get("rows"), list) else []:
        if row.get("status") == "PASS_WITH_WARNINGS":
            add_issue(
                ...
                "MEDIUM",
                "PROOF_GATE_WARNING",
                f"Proof gate warning: {row.get('name')}",
    ...
    if ev and ev.get("fresh_broker_live_data_proven") is not True:
        add_issue(
            ...
            "Fresh broker live data is not proven in proof pack.",
            {"reason": ev.get("reason_fresh_broker_live_data_not_proven")},
        )
    ...
    if lev.get("full_lifecycle_proven") is not True or lev.get("lifecycle_proof_dry_run") is True:
        add_issue(
            ...
            "Real market analyzer paper lifecycle is not proven.",
            {"full_lifecycle_proven": lev.get("full_lifecycle_proven"), "dry_run": lev.get("lifecycle_proof_dry_run")},
        )
    ...
    if raw.get("dry_run") is True or raw.get("signal", {}).get("instrument_token") == "DRY_RUN_TOKEN":
        add_issue(
            issues,
            "CRITICAL",
            "RAW_LIFECYCLE_IS_DRY_RUN",
            "Raw lifecycle proof is dry-run/simulation.",
            {
                "market_status": raw.get("market_status"),
                "instrument_token": raw.get("signal", {}).get("instrument_token"),
            },
        )
    ...
    if mev.get("promotion_allowed") is not True:
        add_issue(
            ...
            "Model promotion is blocked by policy/proof.",
            {"promotion_allowed": mev.get("promotion_allowed")},
        )
...
def summarize(snapshot: dict) -> dict:
    state = (
        snapshot.get("live", {}).get("state", {}).get("data", {})
        if snapshot.get("live", {}).get("state", {}).get("ok")
        else {}
    )
    health = (
        snapshot.get("live", {}).get("health", {}).get("data", {})
        if snapshot.get("live", {}).get("health", {}).get("ok")
        else {}
    )
