...
class ThresholdTuner:
    ...
    def _load_pnl_for_signals(self, df_signals: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            pnl_path = _root() / "storage" / "live" / "dhan_index_ai_pnl_log.csv"
...
def auto_update_trade_config() -> bool:
    """
    Read auto-tuned thresholds JSON and update dhan_trade_config.py.
    ...
    from core.engine import dhan_trade_config

    dhan_trade_config.DEFAULT_THRESHOLDS.min_confidence = float(avg_conf)
    dhan_trade_config.DEFAULT_THRESHOLDS.min_abs_score = float(avg_score)
...
def main() -> None:
    ...
    print("=== Dhan Index Options Threshold Tuner ===")
