...
MODELS_DIR = _project_root() / "core" / "models" / "dhan"
...
def run_once_with_snapshot(df_snap: pd.DataFrame, use_new_engine: bool = True) -> pd.DataFrame:
    ...
    cfg = dhan_trade_decision.TradeConfig()
    plan_df = dhan_trade_decision.build_trade_plan(df_signals, cfg)
    dhan_trade_decision.print_trade_summary(plan_df)
    dhan_trade_decision.append_trade_plan(plan_df)
    logger.info(
        "AI trade plan built: rows=%d, saved_to=%s",
        0 if plan_df is None else len(plan_df),
        str(dhan_trade_decision._trades_csv()),
    )
...
def main() -> None:
    root = _project_root()
    watch_csv = root / "storage" / "live" / "dhan_index_options_watch.csv"
