        ...
        if paper_trades_file.exists():
            try:
                ...
                if 'strategy' in df.columns:
                    strategy_analysis = df[df['action'] == 'CLOSE'].groupby('strategy')['realized_pnl'].agg(['count', 'sum', 'mean'])
    ...
    def analyze_performance_metrics(self) -> Dict:
        ...
        if paper_trades_file.exists():
            try:
                ...
                print(f"[OK] Performance analysis: {results['total_trades']} trades, {results['win_rate']:.2%} win rate")
            except Exception as e:
