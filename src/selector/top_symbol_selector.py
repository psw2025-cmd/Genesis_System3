...
class TopSymbolSelector:
    ...
    def __init__(self, spread_threshold_pct: float = 6.0, min_volume_near_atm: int = 1000, min_strikes_band: int = 10):
        """
        Initialize selector.
    ...
    def calculate_expected_move(self, spot: float, iv: float, time_to_expiry: float) -> float:
        """
    ...
    def liquidity_gate(self, df: pd.DataFrame, spot: float, expected_move: float) -> Tuple[bool, List[str]]:
        """
        ...
        near_atm = df[(df["strike"].notna()) & (abs(df["strike"] - spot) <= atm_band) & (df["volume"].notna())]
        ...
        in_band = df[(df["strike"].notna()) & (abs(df["strike"] - spot) <= strike_band)]
    ...
    def calculate_pcr(self, df: pd.DataFrame, spot: float, band_pct: float = 0.02) -> Tuple[float, float]:
        """
        ...
        # Filter near ATM
        near_atm = df[(df["strike"].notna()) & (abs(df["strike"] - spot) <= band)]
    ...
    def calculate_signal_strength(self, df: pd.DataFrame, spot: float) -> float:
        """
    ...
    def calculate_execution_quality(self, df: pd.DataFrame) -> float:
        """
    ...
    def score_underlying(self, df: pd.DataFrame, underlying: str, spot: float, time_to_expiry: float) -> Dict:
        """
        Score an underlying.
        ...
        underlying_score = signal_strength * 0.5 + execution_quality * 0.3 + (100 if passed else 0) * 0.2
    ...
    def select_top_underlying(
        self, all_data: Dict[str, pd.DataFrame], spots: Dict[str, float], time_to_expiry_map: Dict[str, float]
