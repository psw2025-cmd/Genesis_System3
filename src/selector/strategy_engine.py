...
class StrategyEngine:
    ...
    def analyze_sentiment(self, df: pd.DataFrame, spot: float, pcr: float, delta_pcr: float) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "sentiment": (
                ...
                else "BEARISH" if bearish_score > bullish_score + 10 else "NEUTRAL"
            ),
        }
    ...
    def select_strikes(
        ...
        if strategy_type in ["debit", "buy"]:
            ...
            strikes = (
                df_filtered[(df_filtered["strike"] >= spot - band) & (df_filtered["strike"] <= spot + expected_move)][
                    "strike"
                ]
                .unique()
                .tolist()
            ...
            # For selling: OTM
            strikes = df_filtered[df_filtered["strike"] > spot + expected_move * 0.3]["strike"].unique().tolist()

        elif strategy_type == "spread":
            # For spreads: ATM ± expected_move
            strikes = (
                df_filtered[
                    (df_filtered["strike"] >= spot - expected_move) & (df_filtered["strike"] <= spot + expected_move)
                ]["strike"]
                .unique()
                .tolist()
            )
            ...
            band = expected_move
            strikes = (
                df_filtered[(df_filtered["strike"] >= spot - band) & (df_filtered["strike"] <= spot + band)]["strike"]
                .unique()
                .tolist()
            )
    ...
    def recommend_strategy(
        ...
        for strike in strikes[:2]:  # Max 2 strikes for simplicity
            contract = df[
                (df["strike"] == strike) & (df["option_type"] == option_type if option_type != "BOTH" else True)
            ]
