...
NOTE: Dhan broker path is disabled (System3 is Dhan-only).
LiveChainREST will raise RuntimeError at runtime when the broker
tries to execute any Dhan API call via the disabled shim.
"""
...
class LiveChainREST:
    ...
    def fetch_option_chain_batch(
        ...
        try:
            ...
            if expiry_date:
                option_chain = [
                    opt for opt in option_chain if str(opt.get("expiry", "")).upper() == expiry_date.upper()
                ]
            ...
            if expected_move and option_chain:
                ...
                if spot > 0:
                    ...
                    option_chain = [opt for opt in option_chain if abs(opt.get("strike", 0) - spot) <= strike_band]
            ...
            if strikes:
                option_chain = [opt for opt in option_chain if opt.get("strike") in strikes]
    ...
    def fetch_all_indices(self, indices: List[Dict], expiry_map: Dict[str, str]) -> Dict[str, List[Dict]]:
        """
