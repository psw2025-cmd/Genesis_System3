...
from core.brokers.dhan.instruments import load_instruments
...
def select_expiry_for_underlying(
    underlying_name: str, exchange: str = "NFO", prefer_weekly: bool = True
    ...
    if prefer_weekly:
        ...
        if not current_month_expiries.empty:
            ...
            expiry_str = current_month_expiries[current_month_expiries["expiry_dt"] == monthly_expiry][expiry_col].iloc[
                0
            ]
...
def get_expiry_for_all_indices(indices: list, prefer_weekly: bool = True) -> dict:
    """
    ...
    for idx in indices:
        ...
        expiry_date, expiry_str, reason = select_expiry_for_underlying(name, exchange, prefer_weekly)
        results[name] = {
...
class ExpirySelector:
    ...
    def get_nearest_weekly_expiry(self, underlying_name: str, exchange: str = "NFO") -> Optional[str]:
        ...
        """
        expiry_date, expiry_str, reason = select_expiry_for_underlying(underlying_name, exchange, prefer_weekly=True)
        if expiry_str:
