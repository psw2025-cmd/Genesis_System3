...
def compute_deltas(
    ...
    key_cols: list = ['token', 'strike', 'option_type']
) -> pd.DataFrame:
    """
    ...
    if 'oi_prev' in merged.columns:
        ...
        result['dOI_pct'] = ((merged['oi'] - merged['oi_prev']) / merged['oi_prev'] * 100).where(merged['oi_prev'] > 0, None)
    else:
        result['dOI'] = None
        result['dOI_pct'] = None
    ...
    if 'volume_prev' in merged.columns:
        ...
        result['dVolume_pct'] = ((merged['volume'] - merged['volume_prev']) / merged['volume_prev'] * 100).where(merged['volume_prev'] > 0, None)
    else:
        result['dVolume'] = None
    ...
    if 'mid_price_prev' in merged.columns:
        ...
        result['dMid_pct'] = ((merged['mid_price'] - merged['mid_price_prev']) / merged['mid_price_prev'] * 100).where(merged['mid_price_prev'] > 0, None)
    else:
        result['dMid'] = None
    ...
    if 'ltp_prev' in merged.columns:
        ...
        result['dLTP_pct'] = ((merged['ltp'] - merged['ltp_prev']) / merged['ltp_prev'] * 100).where(merged['ltp_prev'] > 0, None)
    else:
        result['dLTP'] = None
    ...
    result['oi_buildup'] = result.apply(
        lambda row: classify_oi_buildup(
            row.get('dLTP', 0),
            row.get('dOI', 0),
            row.get('dLTP_pct'),
            row.get('dOI_pct')
