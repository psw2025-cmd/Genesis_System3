...
class QCValidator:
    ...
    def validate_snapshot(
        ...
        if 'strike' in df.columns and 'spot_price' in df.columns:
            ...
            if spot:
                ...
                if len(strikes_near_atm) < min_strikes:
                    failures.append(f"Only {len(strikes_near_atm)} strikes near ATM (need >= {min_strikes}, band={atm_band/spot*100:.1f}%)")
        ...
        if 'fetch_timestamp' in df.columns:
            ...
            try:
                df['ts_parsed'] = pd.to_datetime(df['fetch_timestamp'].str.replace(' IST', ''), format='%Y-%m-%d %H:%M:%S', errors='coerce')
