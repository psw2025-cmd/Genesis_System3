...
ScenarioType = Literal[
    "TREND_UP", "TREND_DOWN", "RANGE", "HIGH_VOL", "LOW_LIQUIDITY", "DATA_ERROR", "WS_FAIL", "PARTIAL_FAILURE"
]
...
class ReplayEngine:
    ...
    def generate_snapshot(
        self, scenario: ScenarioType, cycle: int, total_cycles: int, inject_errors: bool = False
    ...
    def _apply_scenario(
        self, df: pd.DataFrame, scenario: ScenarioType, progress: float, inject_errors: bool, cycle: int = 0
    ) -> pd.DataFrame:
        ...
        for idx in df.index:
            ...
            theoretical_price = black_scholes_price(spot, strike, time_to_exp, 0.06, iv, option_type)
            ...
            if inject_errors and random.random() < 0.15:  # 15% error rate
                error_type = random.choice(
                    ["ask_below_bid", "missing_iv", "missing_oi", "nan_field", "stale_timestamp"]
                )
