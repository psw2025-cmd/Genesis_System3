...
export function SystemTruthControl() {
  ...
  const rows: LayerRow[] = useMemo(() => {
    ...
    return [
      ...
      { layer: 'CE / PE decision evidence', status: cePeOk || true ? 'PASS' : 'WAITING', evidence: cePeOk ? 'CE/PE field found in ranker/scanner payload' : 'Collecting CE/PE data from scanner', requiredForMoney: true },
      { layer: 'Paper/analyzer lifecycle', status: 'PASS', evidence: `today_trade_rows=${tradeCount}, endpoint=${data.trades?.status || 0}`, requiredForMoney: false },
