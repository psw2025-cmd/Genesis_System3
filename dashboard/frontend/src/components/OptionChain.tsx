...
function SymbolControls({ chainSymbol, setChainSymbol, universe, discovery, discoveryError }: {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!next) return
    if (universe.length > 0 && !universe.includes(next)) return
    choose(next)
  }

  return (
    <div className="flex flex-wrap items-center gap-2 px-4 py-2 border-b border-border bg-surface-1 flex-shrink-0">
      <div className="flex flex-wrap gap-1">
        {indexSymbols.map(sym => <button key={sym} onClick={() => choose(sym)} className={cn('px-2.5 py-1 rounded text-[11px] font-mono font-semibold transition-colors', chainSymbol === sym ? 'bg-accent text-white' : 'bg-surface-2 text-text-secondary hover:text-text-primary border border-border')}>{sym}</button>)}
      </div>
...
export function OptionChain() {
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/underlyings`, { credentials: 'include', headers: { Accept: 'application/json', ...API_HEADERS }, signal: ctrl.signal })
      .then(async response => { if (!response.ok) throw new Error(`HTTP_${response.status}`); return response.json() })
      .then(payload => {
        if (cancelled) return
  ...
  useEffect(() => { if (atmRef.current) atmRef.current.scrollIntoView({ block: 'center', behavior: 'smooth' }) }, [data?.spot, chainSymbol, range])
  ...
  const atmIdx = strikes.length > 0 ? strikes.reduce((best, strike, index) => Math.abs(strike - spot) < Math.abs(strikes[best] - spot) ? index : best, 0) : 0
  ...
  const streamLabel = streamLive ? 'LIVE DHAN' : (marketOpen ? 'POLLING / DEGRADED' : 'SESSION SNAPSHOT')
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-wrap items-center gap-4 px-4 py-2 border-b border-border bg-surface-1 flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5"><span className="text-text-muted text-xs">SYMBOL</span><span className="num text-sm font-semibold">{chainSymbol}</span></div>
        <div className="flex items-center gap-1.5"><span className="text-text-muted text-xs">SPOT</span><span className="num text-sm font-semibold">{spot ? fmt(spot, 2) : '--'}</span></div>
        <div className="flex items-center gap-1.5"><span className="text-text-muted text-xs">PCR</span><span className="num text-sm font-semibold">{typeof pcr === 'number' ? pcr.toFixed(2) : pcr}</span></div>
        <div className="flex items-center gap-1.5"><span className="text-text-muted text-xs">CONTRACTS</span><span className="num text-xs">{contracts.length}</span></div>
        <div className="flex items-center gap-1.5"><span className="text-text-muted text-xs">STRIKES</span><span className="num text-xs">{strikes.length}</span></div>
        <div className="ml-auto flex items-center gap-2"><span className="text-text-muted text-[10px]">VISIBLE</span><select value={range} onChange={e => setRange(Number(e.target.value))} className="bg-surface-2 border border-border rounded px-2 py-1 text-xs text-text-secondary"><option value={0}>ALL STRIKES ({strikes.length})</option>{[5, 10, 20, 40].map(n => <option key={n} value={n}>+/-{n} ATM</option>)}</select></div>
      </div>
      <div className={cn('px-4 py-2 border-b border-border text-[10px] font-mono', stale || !streamLive ? 'text-amber bg-amber/5' : 'text-text-muted')}>
        source={String(dataSource)} priority={String(sourcePriority)} status={String(status)}{snapshotAge != null ? ` age=${snapshotAge}s` : ''}{fetchedAt !== '--' ? ` fetched=${String(fetchedAt)}` : ''}{discovery?.source ? ` universe=${String(discovery.source)}` : ''}{data?.message ? ` - ${String(data.message)}` : ''}
      </div>
      ...
      {contracts.length === 0 ? (
        <div className="flex-1 overflow-auto p-4"><div className="card p-4"><div className="panel-title">Option Chain</div><div className="mt-3 text-amber font-semibold">NO VERIFIED BROKER CHAIN ROWS</div><div className="mt-2 text-xs text-text-muted leading-relaxed">{String(data?.message || marketReason || 'Waiting for a Dhan-backed option-chain snapshot.')}</div><div className="mt-3 grid gap-2 md:grid-cols-3 text-xs"><div><span className="text-text-muted">Selected:</span> {chainSymbol}</div><div><span className="text-text-muted">Universe:</span> {discovery?.counts?.total ?? universe.length} underlyings</div><div><span className="text-text-muted">Safety:</span> ANALYZER / PAPER · LIVE OFF</div></div></div></div>
      ) : (
        <div className="flex-1 overflow-auto"><table className="w-full border-collapse text-xs"><thead className="sticky top-0 z-10 bg-surface-1"><tr>{['OI','ChgOI','Vol','IV','LTP','Bid','STRIKE','Ask','LTP','IV','Vol','ChgOI','OI'].map((h,i)=><th key={`${h}-${i}`} className={cn('thead border-b border-border', i<6?'text-right':i===6?'text-center bg-surface-2 px-4 py-2 text-text-primary font-bold':'text-left')}>{h}</th>)}</tr></thead><tbody>
          {visible.map(strike => {
