...
class UpgradeAgent:
    ...
    def create_upgrade_plan(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for issue in issues:
            if issue["type"] == "log_error":
                plan["changes"].append(
                    {"type": "log_fix", "file": issue.get("file", "unknown"), "action": "Review and fix error in logs"}
                )
                plan["auto_apply"] = True  # Log fixes can be auto-applied

            elif issue["type"] == "backend_unhealthy":
                plan["changes"].append({"type": "backend_restart", "action": "Restart backend service"})
                plan["auto_apply"] = True

            elif issue["type"] == "qc_fail":
                plan["changes"].append(
                    {"type": "qc_investigation", "action": "Investigate QC failure and fix data quality issues"}
                )
        ...
        try:
            ...
            if state_res.status_code == 200:
                test_results["tests"].append(
                    {"name": "SSOT Endpoint", "status": "pass", "message": "SSOT endpoint is accessible"}
                )
            ...
            test_results["tests"].append({"name": "SSOT Endpoint", "status": "fail", "message": str(e)})
            test_results["failed"] += 1
    ...
    def apply_upgrade(self, plan: Dict[str, Any], test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if test_results["failed"] > 0:
            return {"success": False, "message": "Tests failed, cannot apply upgrade", "test_results": test_results}

        if not plan.get("auto_apply", False):
            return {"success": False, "message": "Upgrade requires manual approval", "plan": plan}
        ...
        with zipfile.ZipFile(proof_pack_file, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            if state_history_dir.exists():
                recent_snapshots = sorted(
                    state_history_dir.glob("*.json"), key=lambda f: f.stat().st_mtime, reverse=True
                )[:10]
