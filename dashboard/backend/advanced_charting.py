...
class AdvancedCharting:
    ...
    def generate_option_chain_heatmap(self, chain_data: Dict[str, Any], metric: str = "oi") -> Dict[str, Any]:
        """
        ...
        for expiry in expiries:
            ...
            for strike in strikes:
                ...
                matching = [c for c in contracts if c.get("strike") == strike and c.get("expiry") == expiry]
                ...
                row.append(value)
            heatmap_data.append({"expiry": expiry, "values": row})
    ...
    def generate_iv_surface(self, chain_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for expiry in expiries:
            ...
            for strike in strikes:
                ...
                row.append(avg_iv)
            surface_data.append({"expiry": expiry, "iv_values": row})

        return {"surface": surface_data, "strikes": strikes, "expiries": expiries, "spot": chain_data.get("spot", 0)}

    def generate_greeks_chart(self, chain_data: Dict[str, Any], greek: str = "delta") -> Dict[str, Any]:
        """
    ...
    def generate_pcr_chart(self, chain_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        pcr_data = []
        for strike in strikes:
            ce_oi = sum(c.get("oi", 0) for c in contracts if c.get("strike") == strike and c.get("option_type") == "CE")
            pe_oi = sum(c.get("oi", 0) for c in contracts if c.get("strike") == strike and c.get("option_type") == "PE")
    ...
    def generate_equity_curve_enhanced(self, pnl_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
