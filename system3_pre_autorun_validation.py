...
def phase_c_live_day_simulation():
    ...
    simulation_events.append(
        {"time": "09:15", "event": "Autopilot start", "expected": "DRY-RUN autopilot starts", "status": "simulated"}
    )

    # 09:45 - First 30-min phase run
    simulation_events.append(
        {"time": "09:45", "event": "30-min interval", "expected": "Phases 220-260 run", "status": "simulated"}
    )

    # 10:00 - First OP cycle
    simulation_events.append(
        {"time": "10:00", "event": "Hourly OP cycle", "expected": "OP1, OP2, OP3 run", "status": "simulated"}
    )
    ...
    simulation_events.append(
        {"time": "15:30", "event": "Archive signals", "expected": "Signals archived", "status": "simulated"}
    )

    # 15:35 - EOD learning
    simulation_events.append(
        {"time": "15:35", "event": "EOD learning", "expected": "End-of-day learning runs", "status": "simulated"}
    )
    ...
    for event in simulation_events:
        report_lines.append(f"| {event['time']} | {event['event']} | {event['expected']} | {event['status']} |")
