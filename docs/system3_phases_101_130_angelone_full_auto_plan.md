...
## Angel One Full Auto Layer – Mode 1 (Indian Market Only)
...
This plan adds **Angel One LIVE trading automation** (Mode 1 only), in a **safe, layered, fully-configurable** way.
...
106–110: AngelOne Execution Wrapper & Dry-Run Mirroring
...
PHASE 103 – AngelOne Low-Level Order Wrapper (Skeleton)

Goal: Abstract AngelOne SmartAPI order placement into a dedicated module.

Files

Create core/broker/angel_live_order_wrapper.py
...
NO actual SmartAPI calls in this phase.
...
Call existing DRY RUN executor module (already in System3; name may be angel_trade_executor_dryrun or similar – Cursor agent must inspect existing code and reuse).
...
PHASE 107 – AngelOne LIVE Execution (ONE-LOT, GUARDED, OFF BY DEFAULT)
...
Reuse core/broker/angel_live_order_wrapper.py implemented in Phase 103.
...
core/broker/angel_live_order_wrapper.py (still likely DRY_RUN or stub).
...
Pritam will later decide when to plug in real SmartAPI code.
...
Read angel_index_ai_trades_plan.csv and live_orders_ledger.csv.
