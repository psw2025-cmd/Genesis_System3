...
**Last upgraded:** 2026-08-27 01:35 IST (rebase #373 MRI plan onto full cross-verify #4 / #372)
...
## 0A) Visible progress board (2026-08-27 01:35 IST) — full cross-verify #4 + MRI 5-min watch — DO NOT SKIP

**Evidence:** `reports/latest/repo_path_audit/cloud_github_vs_laptop.json` + `reports/latest/full_cross_verify_20260826_193000/` + `reports/latest/mri_watch/LATEST.json`
...
**MRI plan:** `docs/handoffs/MRI_GMAIL_SCHEDULER_5MIN_CONTROL_PLAN.md` + watcher `scripts/system3_mri_gmail_scheduler_watch.py`
**Sibling lanes:** full cross-verify + MRI watch — do not compete; rebase docs carefully

### Same-session live truth (Cloud + GitHub — NOT laptop)

| Item | Value |
|---|---|
| GitHub `origin/main` | `1114ba3ab5afd29202482a151b654382e14de76c` — **#372** tip (includes #371/#370/#369) |
...
| Scheduler health | **HEALTHY** transport · business **PARTIAL** (wrong-date rank/forecast/signals) · MRI tick **WARN** |
...
| Gmail intake | **DONE** — full-verify 25 threads + MRI tick `gmail_latest.json` |
...
5. Landed MRI Gmail+Scheduler 5-min control plan + watcher; recurrence via Task Scheduler / `--loop` (no Actions `schedule:`)

### Do not (this cycle)

- Redeploy solely to catch docs/test tip
...
- Invent a competing 5-min Gmail/scheduler plan (watcher owns continuous ticks)
...
| P0-MRI | 5-min Gmail+Scheduler watch | **SCRIPT DONE** — arm Task Scheduler / `--loop`; ticks → `reports/latest/mri_watch/` |

### Need from user

**None required for ruleset/MRI plan.** Optional: register Windows task (`reports/latest/mri_watch/RECURRENCE.md`); Cursor GitHub App later.

### Agent next (no user wait)

1. Squash-merge #373 when CI green; delete branch
2. Arm local 5-min MRI watch; confirm `LATEST.json` tick
...
- No GitHub Actions `schedule:` for MRI recurrence  
