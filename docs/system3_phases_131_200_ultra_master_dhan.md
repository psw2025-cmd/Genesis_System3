...
Master implementation + cursor-agent instructions (Dhan-only, DRY-RUN safe)
...
> System3 Ultra Master phases 131–200 in a **safe**, **Dhan-only**,
...
1) Broker scope
   - ONLY Dhan is allowed.
   - Do NOT import or reference Binance, crypto, or any other broker.
   - If you need broker-related info, always use the existing Dhan layers
     already in `core/engine` and `core/models/dhan`.
...
2) DRY-RUN safety
   ...
   - Never send anything to DhanHQ that places, modifies, or cancels an order.
...
- Reads:
  ...
  - Existing health/info from:
    - `core/engine/dhan_health_check.py` (if exists)
    - Or run a light inline Dhan capability check using existing APIs in safe mode.
...
2. Gather minimal health info:
   ...
   - Dhan connectivity: try a **SAFE** call (like get profile or holdings) but DO NOT place orders.
3. Classify status:
...
Function:
- `run_phase136_dhan_symbol_universe()`
...
Outputs:
- CSV: `storage/ultra/phase136_dhan_symbol_universe.csv`
- JSON: `storage/ultra/phase136_dhan_symbol_universe.json`
...
Inputs:
- `phase136` universe
- Optionally an Dhan safe method to fetch expiries (if available in dry API).
