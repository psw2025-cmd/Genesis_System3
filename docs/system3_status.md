...
- [x] Trade rule engine (dhan_trade_rules.py)
  - Meta-rule evaluation engine
  - Signal → Action mapping
  - Rule confidence blending
  - Integrated into dhan_trade_decision.py

- [x] Multi-resolution labels (dhan_multi_resolution_labels.py)
  - Generates label_1, label_2, label_3, label_5
  - Integrated into train_dhan_models.py

- [x] Advanced feature engineering (dhan_features.py)
  - 25+ new features (moneyness_pct, dist_atm_pct, premium_decay_speed, etc.)
  - Integrated into generate_synthetic_dhan_training.py

- [x] Model training with MI-based feature selection (train_dhan_models.py)
  - Trains per-underlying models with 98–100% synthetic accuracy
  - Uses top 20 MI-selected features per underlying

- [x] Live AI signals engine (dhan_live_ai_signals.py)
  - Loads models and generates predictions
  - Logs signals to CSV
  - Integrated into menu option 11

- [x] Trade decision layer (dhan_trade_decision.py)
  - Uses TradeRuleEngine for filtering
  - Generates trade plans with entry/target/SL
  - Logs to CSV

- [x] PnL simulator (dhan_pnl_simulator.py)
...
- [x] Threshold tuner (dhan_threshold_tuner.py)
  ...
  - Updates dhan_trade_config.py automatically

- [x] Synthetic training generator (generate_synthetic_dhan_training.py)
...
- [x] Synthetic backtester (dhan_synthetic_backtester.py)
