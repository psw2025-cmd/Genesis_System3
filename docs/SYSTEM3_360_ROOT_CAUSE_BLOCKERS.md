...
Reason: UI visibility is improving, but real trading chain is still blocked at broker auth, option-chain universe, scanner/ranker, paper lifecycle, ML/training proof, Cloud Run/fresh visual proof, GitHub failure tracking, and final truth aggregation. Render.com hosting is forbidden (retired host).

## Permanent operating authority and safety boundary

User grants standing authority to execute repo repair, dependency install checks, GitHub Actions proof workflows, Cloud Run verification, dashboard visual proof, integration verification, and TODO/status reporting required to resolve System3 blockers.
...
No issue, feature, fix, gate, workflow, model, broker state, paper state, scanner state, or production-grade status may be claimed DONE / fixed / resolved / passed unless the live GCP Cloud Run dashboard UI (`https://genesis-system3-web-doq2wplepa-el.a.run.app/ui/`) is visually checked by automation after the latest relevant commit and shows the required proof text on screen. Render.com is not current proof.
...
2. Cloud Run can show stale frontend/backend if the latest commit is not deployed and visually verified.
...
10. GitHub workflow failures and Cloud Run public endpoint failures must be tracked together and resolved from proof, not from chat memory. Render.com hosting is forbidden.
...
|---|---|---|---|---|
| B01 | Dhan token/auth invalid or expired | User/GCP | Refresh/update Dhan read-only API credentials via the canonical Cloud Run / Secret Manager rotation Job | Broker panel shows auth OK; funds API responds without auth error |
| B02 | Dhan client/account identity not confirmed by broker API | User/GCP | Confirm correct Dhan client/account credentials are configured | Broker status shows valid client/account proof or clear broker limitation |
...
| B04 | Cloud Run deploy may be stale | GCP/GitHub | Verify serving SHA equals latest expected commit | `/api/deploy/info` proof plus visual screenshot after deploy |
...
| B07 | Dhan option-chain enabled universe 0/4 | User/Code/GCP | Fix broker auth first, then verify Dhan option-chain per enabled symbol | Truth Control shows enabled universe rows and Dhan chain proof |
...
| B16 | Production Proof Bar not yet visually validated | UI/GCP | Capture the live Cloud Run UI and screenshots | Screenshot shows 7 proof gates visible |
| B17 | Cloud worker/scheduler proof not complete | GCP/Workflow | Worker/Job must write latest proof artifacts reliably | Reports show latest worker/scheduler proof timestamp/status |
| B18 | 360 integration gate missing | Workflow | One workflow must combine Cloud Run API, dashboard visual proof, broker truth, chain truth, scanner, paper, ML, and final verdict | Integration report PASS/BLOCKED with exact blocker list |
...
| B20 | Credential presence/format not continuously proven | Workflow/GCP | Check required credential presence via secure workflow env only; never print values | credential audit report with redacted presence/format status |
...
| B22 | GitHub workflow failure tracker not proven PASS | Workflow/GCP | Track GitHub failed runs and Cloud Run public endpoint failures together | `docs/SYSTEM3_WORKFLOW_FAILURE_TODO.md` and `reports/latest/workflow_failure_tracker/summary.json` PASS |
...
2. Verify correct Dhan client ID/account is configured in GCP Secret Manager / Cloud Run when audit says missing/mismatch.
3. Do not paste secrets in chat.
4. After updating Cloud Run secrets/env, redeploy `genesis-system3-web` (never Render).
...
3. Centralize public truth: latest commit, Cloud Run serving SHA, visual proof commit, and dashboard UI must match.
...
9. Track GitHub workflow failures and Cloud Run public endpoint failures in one persistent TODO.

## Cloud Run / workflow fixes required
...
8. Verify GitHub workflow-failure tracker PASS. Render hosting is forbidden.

## Required final proof before any resolved claim

1. Cloud Run deploy commit / serving SHA proof.
2. Broker auth proof.
...
11. GitHub workflow-failure tracker PASS.
